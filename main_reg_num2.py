...
def get_distribution(labels):
    ...
    assert (max(labels)*100).item()<100
    
    for label in labels:
        mean, sigma= (label*100).int().item(), 100
...
def train(model, device, train_loader, optimizer, epoch, log_interval, args):
    ...
    for batch_idx, data in enumerate(train_loader):
        ...
        fusion_features = model(data)

        gt_sentence_features = generate_samples(model, data, 0, 100)
        
        
        labels = torch.arange(data.y.shape[0]).long().to(device) 
        
        asd = (data.y*100).int().cpu().numpy()
        
        text_logits = gt_sentence_features[asd,:]
...
def predicting(model, device, loader, loader_type, args):
    ...
    with torch.no_grad():
        for batch_idx, data in tqdm(enumerate(loader)):
            ...
            pred = (model.logit_scale.exp()*torch.bmm(gt_sentence_features,fusion_features)).squeeze()
            pred = (torch.argmax(pred,1)/100).cpu().view(-1, 1)
