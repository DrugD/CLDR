...
#         for ic50 in range(0,100,1):
#             # pdb.set_trace()
#             des = "zero point" + num2english(ic50/100)
#             temp_number.append(des)
#             des_ = "The drug response value between " + data.smiles[index] + " and "+ data.cell_name[index] +" is "
#             temp_text.append(des_)
            
#         # descriptions_number.append(temp_number)
#         # descriptions_text.append(temp_text)
        
        
#         desc_number = clip.tokenize(temp_number,context_length=300).to(device)
#         desc_text = clip.tokenize(temp_text,context_length=300).to(device)
    
#         # pdb.set_trace()
#         gt_text_features = model.encode_text(desc_text)
#         gt_number_features = model.encode_num(desc_number)
#         gt_sentence_features = torch.cat((gt_text_features,gt_number_features),axis=1)
#         gt_sentence_features = model.transformer_fusion(gt_sentence_features)
#         sentence_feature.append(gt_sentence_features)
...
def generate_samples(model, data):
    ...
    # for ic50 in range(0,100,1):
    #     des = "The drug response value between " + data.smiles[index] + " and "+ data.cell_name[index] +" is "+ str(ic50/100) + "."
    #     temp.append(des)
...
# descriptions_text.append(temp_text)
    ...
    text_features = model.encode_num(text)
        # sentence_feature.append(text_features)
    # pdb.set_trace()        
...
def main(config, yaml_path):
    ...
    train_loader = DataLoader(train_dataset, batch_size=config['batch_size']['train'], shuffle=True, drop_last=False)
