...
def train(model, device, train_loader, optimizer, epoch, log_interval, args):
    ...
    for batch_idx, data in enumerate(train_loader):
        ...
        # pdb.set_trace()
        ng_sample_time = int(text_logits.shape[0]/fusion_features.shape[0])
        
        labels = torch.arange(data.y.shape[0]).long().to(device) 
        
        labels_ = torch.repeat_interleave(labels, ng_sample_time ,dim=0)
        
        # pdb.set_trace()
        drug_cell_logits = model.logit_scale * fusion_features @ text_logits.t()
        
        loss_dc = DC_cross_entropy_loss(drug_cell_logits, labels*ng_sample_time)

        loss_t = T_cross_entropy_loss(drug_cell_logits.t()[labels*ng_sample_time], labels)
        ...
        if batch_idx % log_interval == 0:
            print(
                "Train epoch: {} ({:.0f}%)\tloss_dc: {:.6f}\tloss_t: {:.6f}\tloss_CLIP: {:.6f}".format(
                    epoch, 100.0 * batch_idx / len(train_loader), loss_dc.item(), loss_t.item(), loss_CLIP.item()
                )
            )
...
def generate_samples(model, data, start, end, index):
    ...
    for idx, ic50 in enumerate(index):
        ...
        des1 = "The drug with " + data.cell_name[idx] + " is zero point " + num2english(ic50/1000)
        des2 = "The drug with " + data.cell_name[idx] + " is zero point " + num2english(random.randint(0, ic50)/1000)
        des3 = "The drug with " + data.cell_name[idx] + " is zero point " + num2english(random.randint(ic50, 1000)/1000)

        descriptions.extend([des1,des2,des3])
