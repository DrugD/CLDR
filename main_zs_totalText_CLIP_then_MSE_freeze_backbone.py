...
def train(model, device, train_loader, optimizer, epoch, log_interval, args):
    ...
    for batch_idx, data in enumerate(train_loader):
        ...
        output_ic50, fusion_features = model(data)
...
def main(config, yaml_path):
    ...
    model.load_state_dict(torch.load(
        "/home/lk/project/DALLE24Drug/CLIP4Drug/CLIP_DRP/exp/GAT_GCN_CLIP_Total_Text/GDSCv2_zs__20231017163701/GraphDRP.model", map_location=torch.device(device)), strict=True)
    freeze_model(model)
    model.to(device)
