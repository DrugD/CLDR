...
drug + cell = F1
text(number 2 text) = F2

然后不采用CLIP的F1 和 F2相互计算cos相似度,而是采用F1直接去和所有
text(1~1000 2 text) = F2(1~1000)
做匹配,按照

以label为 miu,以10/10000, 为sigam 的高斯分布
和 F2(10000,128)@F1(128,1)= Pred(10000,1)进行分布差异约束

这样尝试的意图是避免 F1 和 例如label是0.1234/0.1235 这两者之间的差距无法感知
导致模型按照分类任务去做，对数字的感知能力和表达下降。
...
训练阶段: Feature of drug+cell ~ WordVec of zero point one two three four
预测阶段: Feature of drug+cell @ WordVec(DEC).T() => 配合 离散的1~1000这么多个类别 做分类预测
'''
